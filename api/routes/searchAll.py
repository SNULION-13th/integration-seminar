...
# async def search_all_items(request: Request):
#     try:
#         # MySQL에서 모든 항목을 최신순으로 가져오기
#         items = models.DashboardItem.order_by(models.DashboardItem.created_at.desc()).all()
#         results = [
#             DashboardItemResponse.from_orm(item)
#             for item in items
#         ]
